...
#################################################################
#added by JIA
setenv year1 2016               # Target Year

foreach jdate (`seq 157 1 157`)

if (($year1 % 4) != 0) then
   set leap_year_cur_year = 0
else if (($year1 % 400) == 0) then
   set leap_year_cur_year = 1     # leap year, nday = 29
else if (($year1 % 100) == 0) then
   set leap_year_cur_year = 0
else
   set leap_year_cur_year = 1     # leap year, nday = 29
endif
...
while ($t_days < $jdate)
        @ t_days = $t_days + $numday_cur_year[$t_month]
        @ t_month++
end
@ t_month--

 @ c_days = $t_days - $numday_cur_year[$t_month]
 @ month1 = $t_month
 @ day1 = $jdate - $c_days


echo Target Julian Date : $jdate
echo Target Date : $year1 - $month1 - $day1

 if ($jdate >= 100) then
  set TJDATE   = ${year1}${jdate}
 else if ($jdate >= 10) then
  set TJDATE   = ${year1}0${jdate}
 else
  set TJDATE   = ${year1}00${jdate}
 endif
...
while ($p_days < $p_date)
        @ p_days = $p_days + $numday_cur_year[$p_month]
        @ p_month++
end
@ p_month--
...
#> Set General Parameters for Configuring the Simulation
 ...
 set APPL      = EASIA         #> Application Name (e.g. Gridname)
...
#> Set Working, Input, and Output Directories
 ...
 setenv OUTDIR /tng10/users/jjung/OUTPUT_EASTASIA/2016/CMAQ_NO_GEOS #/tng9/users/apouyaei/6TG/CMAQ-5.2/CCTM/scripts/test_output #/tng10/users/jjung/OUTPUT_EASTASIA/2015/CMAQ_NO_GEOS #> Output Directory
 setenv OUTDIR2 /tng9/users/apouyaei/8TG/CMAQ-5.2/CCTM/scripts/test_output #Output Directory
 setenv INPDIR  ${WORKDIR}/input  #> Input Directory
...
#> Set Start and End Days for looping
 ...
 set START_DATE = "${year1}-${month1}-${day1}"     #> beginning date (July 1, 2011)
 set END_DATE   = "${year1}-${month2}-${day2}"     #> ending date    (July 14, 2011)
...
setenv GRID_NAME EASIA        #> check GRIDDESC file for GRID_NAME options
setenv GRIDDESC /tng10/users/jjung/OUTPUT_EASTASIA/2016/MCIP_OUT/GRIDDESC #> grid description file
...
#>   ACONC file species; comment or set to "ALL" to write all species to ACONC
     ...
     setenv AVG_CONC_SPCS  "O3 NO NO2 FORM" #"ALL" 
     setenv ACONC_BLEV_ELEV " 1 ${NZ}" #> ACONC file layer range; comment to write all layers to ACONC
...
setenv PA_BCOL_ECOL "30 320"
setenv PA_BROW_EROW "10 50"
setenv PA_BLEV_ELEV "1  5"

#> I/O Controls
setenv IOAPI_LOG_WRITE F     #> turn on excess WRITE3 logging [ options: T | F ]
setenv FL_ERR_STOP F         #> stop on inconsistent input files
...
setenv AVG_FILE_ENDTIME Y    #> What is this [ default: N ]
...
# =====================================================================
if ($jdate == 1) then
 set ICFILE     = ICON_v52_EASIA_profile
 set ICpath     =  /tng10/users/jjung/OUTPUT_EASTASIA/2016
else
 set ICFILE     = CCTM_CONC_${RUNID}_${PJDATE}.nc
 set ICpath    = $OUTDIR
endif

#set ICpath    = $INPDIR/icbc              #> initial conditions input directory 
#set BCpath    = /tng10/users/jjung/OUTPUT_EASTASIA/2016  #$INPDIR/icbc              #> boundary conditions input directory
#  set BCFILE = BCON_v52_EASIA_profile
set BCpath = /tng6/users/asouri/OI_Project/BCONS
set BCFILE     = BCON_v52_korus_${TJDATE}

set EMISpath  = /tng10/users/jjung/OUTPUT_EASTASIA/2016/EMISSION  #> surface emissions input directory
...
set METpath   = /tng10/users/jjung/OUTPUT_EASTASIA/2016/MCIP_OUT  #$INPDIR/met/mcip          #> meteorology input directory 
...
##  endif
  ...
  #> Boundary conditions
#  set BCFILE = BCON_v52_EASIA_profile#BCON_korus_d1_${TJDATE} #BCON_${YYYYMMDD}_bench.nc
  ...
  setenv GRID_BDY_2D $METpath/GRIDBDY2D_${APPL}_${TJDATE}
  setenv GRID_CRO_2D $METpath/GRIDCRO2D_${APPL}_${TJDATE} #YYMMDD}.nc
  setenv GRID_CRO_3D $METpath/GRIDCRO3D_${APPL}_${TJDATE} #.nc#YYMMDD}.nc
  setenv GRID_DOT_2D $METpath/GRIDDOT2D_${APPL}_${TJDATE} #.nc #{YYMMDD}.nc
  setenv MET_CRO_2D $METpath/METCRO2D_${APPL}_${TJDATE} #.nc #{YYMMDD}.nc
  setenv MET_CRO_3D $METpath/METCRO3D_${APPL}_${TJDATE} #.nc #{YYMMDD}.nc
  setenv MET_DOT_3D $METpath/METDOT3D_${APPL}_${TJDATE} #.nc #{YYMMDD}.nc
  setenv MET_BDY_3D $METpath/METBDY3D_${APPL}_${TJDATE} #.nc #{YYMMDD}.nc

  setenv LAYER_FILE $MET_CRO_3D  # Deprecated: MET_CRO_3D is now read directly in CCTM
 setenv EMISfile EMIS_EASIA_${TJDATE}.ncf
...
##     set EMISfile  = emis_mole_all_${YYYYMMDD}_cb6_bench.nc #> Surface emissions
  ...
  setenv OCEAN_1 /tng10/users/jjung/OUTPUT_EASTASIA/NEW_OCEAN_EASIA.nc #> horizontal grid-dependent surf zone file
...
# =====================================================================
  ...
  setenv CTM_APPL ${RUNID}_${STDATE} ##${YYYYMMDD} 
  ...
  setenv CTM_DRY_DEP_1   "$OUTDIR2/CCTM_DRYDEP_${CTM_APPL}.nc -v"     #> Hourly Dry Deposition
  ...
  setenv CTM_AOD_2       "$OUTDIR/CCTM_AOD2_DIAG_${CTM_APPL}.nc -v"
...
set MPICH = /opt/intel/impi/2018.1.163
set MPI = $MPICH/bin64
set MPIRUN = $MPI/mpirun
set TASKMAP = ./node


#time $MPIRUN -v -machinefile $TASKMAP -np $NPROCS $BLD/$EXEC
